package com.thanhnguyen.cinemaclonecompose.ui.common

import com.thanhnguyen.cinemaclonecompose.ui.model.AccountType
import com.thanhnguyen.cinemaclonecompose.ui.model.Banner
import com.thanhnguyen.cinemaclonecompose.ui.model.Movie

val listBannersData = listOf(
    Banner(
        thumbnail = "https://gamek.mediacdn.vn/thumb_w/600/133514250583805952/2022/3/28/photo-1-1648463807426792939448.jpg",
        title = "Luffy chap 1044",
        content = "Cái huân hòe gỉ đây"
    ),
    Banner(
        thumbnail = "https://animecorner.me/wp-content/uploads/2021/11/one-piece-merged-volumes-covers.jpg",
        title = "Luffy film GOLD",
        content = "Cái huân hòe gỉ đây"
    ),
    Banner(
        thumbnail = "https://media.songdep.com.vn/files/chinguyen/2021/01/26/tom-tat-one-piece-dao-hai-tac-full-tron-bo-152407.jpg",
        title = "Luffy chap 1044",
        content = "Cái huân hòe gỉ đây"
    ),
    Banner(
        thumbnail = "https://c4.wallpaperflare.com/wallpaper/471/183/590/manga-anime-one-piece-monkey-d-luffy-wallpaper-preview.jpg",
        title = "Luffy chap 1044",
        content = "Cái huân hòe gỉ đây"
    ),
    Banner(
        thumbnail = "https://preview.redd.it/m6c3r97t3h271.jpg?width=640&crop=smart&auto=webp&s=25e28ba6fcf6e092daf2b91058549fbcff69f0fb",
        title = "Luffy chap 1044",
        content = "Cái huân hòe gỉ đây"
    )
)

val listMovieHorizontal = listOf(
    Movie(
        1,
        "https://static.wikia.nocookie.net/onepiece/images/f/fd/Movie_10_Poster.png/revision/latest?cb=20130116014223",
        "One Piece Strong World",
        "Cartoon",
        4.2,
        AccountType.Free,
        "2022",
        150,
        storyLine = "Luffy was born in Foosha Village to Monkey D. Dragon and an unknown woman. Dragon left Luffy in the care of his grandfather, Monkey D. Garp, who did many dangerous things to Luffy to make him stronger, like throwing him down a deep ravine, leaving him alone in the wild, and tying him to a balloon."
    ),
    Movie(
        2,
        "https://2.bp.blogspot.com/-B1jLgaPQVVc/VhnZsyA8VDI/AAAAAAAAO2o/orIwD_u207A/s0/One-Piece-Movie-12-Film-Z-Poster-1.jpg",
        "One Piece Film GOLD",
        "Cartoon",
        5.0,
        AccountType.Free,
        "2022",
        150,
        storyLine = "Luffy was born in Foosha Village to Monkey D. Dragon and an unknown woman. Dragon left Luffy in the care of his grandfather, Monkey D. Garp, who did many dangerous things to Luffy to make him stronger, like throwing him down a deep ravine, leaving him alone in the wild, and tying him to a balloon."
    ),
    Movie(
        3,
        "https://gamek.mediacdn.vn/thumb_w/640/133514250583805952/2022/4/14/gut3-16499271078161909466544.jpg",
        "One Piece Film RED",
        "Cartoon",
        4.7,
        AccountType.Free,
        "2022",
        150,
        storyLine = "Luffy was born in Foosha Village to Monkey D. Dragon and an unknown woman. Dragon left Luffy in the care of his grandfather, Monkey D. Garp, who did many dangerous things to Luffy to make him stronger, like throwing him down a deep ravine, leaving him alone in the wild, and tying him to a balloon."
    ),
    Movie(
        4,
        "https://image.lag.vn/upload/news/18/08/26/5-15352610750551894511556_SZGH.png",
        "Onepice chất lượng cao",
        "Cartoon",
        4.8,
        AccountType.Free,
        "2022",
        150,
        storyLine = "Luffy was born in Foosha Village to Monkey D. Dragon and an unknown woman. Dragon left Luffy in the care of his grandfather, Monkey D. Garp, who did many dangerous things to Luffy to make him stronger, like throwing him down a deep ravine, leaving him alone in the wild, and tying him to a balloon."
    ),
    Movie(
        5,
        "https://lh3.googleusercontent.com/-wqLW7DFsWfs/XizQzduiIBI/AAAAAAAAGwE/Fb7kMmkLqLcCJXbF3oucXGqUbF__s5sYQCLcBGAsYHQ/s0/Phimplay_onepiece-1-dao-hai-tac-9.jpg",
        "One Piece Chopper Story",
        "Cartoon",
        4.2,
        AccountType.Free,
        "2022",
        150,
        storyLine = "Luffy was born in Foosha Village to Monkey D. Dragon and an unknown woman. Dragon left Luffy in the care of his grandfather, Monkey D. Garp, who did many dangerous things to Luffy to make him stronger, like throwing him down a deep ravine, leaving him alone in the wild, and tying him to a balloon."
    ),
    Movie(
        6,
        "https://jpsharing.net/wp-content/uploads/2021/11/one-piece-film-red-1.jpg",
        "Nami đẹp vãi chưởng",
        "Cartoon",
        4.2,
        AccountType.Free,
        "2022",
        150,
        storyLine = "Luffy was born in Foosha Village to Monkey D. Dragon and an unknown woman. Dragon left Luffy in the care of his grandfather, Monkey D. Garp, who did many dangerous things to Luffy to make him stronger, like throwing him down a deep ravine, leaving him alone in the wild, and tying him to a balloon."
    ),
)

val highLightMovie =
    Movie(
        5,
        "https://lh3.googleusercontent.com/-wqLW7DFsWfs/XizQzduiIBI/AAAAAAAAGwE/Fb7kMmkLqLcCJXbF3oucXGqUbF__s5sYQCLcBGAsYHQ/s0/Phimplay_onepiece-1-dao-hai-tac-9.jpg",
        "One Piece Chopper Story",
        "Cartoon",
        4.2,
        AccountType.Free,
        "2022",
        150,
        storyLine = "Luffy was born in Foosha Village to Monkey D. Dragon and an unknown woman. Dragon left Luffy in the care of his grandfather, Monkey D. Garp, who did many dangerous things to Luffy to make him stronger, like throwing him down a deep ravine, leaving him alone in the wild, and tying him to a balloon."
    )

val listCategories = listOf(
    "All",
    "Comedy",
    "Animation",
    "Hentai",
    "Document",
)